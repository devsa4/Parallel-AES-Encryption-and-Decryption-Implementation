...
void derive_key_dec(const string &pass, unsigned char key[AES_KEY_LEN]) {
...
bool decrypt_file_multithreaded(const string &in_path,
                                const string &out_path,
                                const string &passphrase,
                                int num_threads) {
    ifstream in(in_path, std::ios::binary);
    ...
    for (int j = 0; j < i; ++j) {
        ...
        if (!td[j].success) {
            cerr << "Decryption failed for chunk " << j << "\n";
            return false;
        }
